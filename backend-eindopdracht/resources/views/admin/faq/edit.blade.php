...
@section('page-title', 'Edit FAQ Item')
@section('page-subtitle', Str::limit($faq->question, 60))

@section('content')

    <form method="POST" action="{{ route('admin.faqs.update', $faq) }}" class="max-w-2xl space-y-6">
        @csrf @method('PATCH')
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="question" value="{{ old('question', $faq->question) }}" required maxlength="500"
            ...
            <div>
                ...
                <textarea name="answer" rows="6" required
                          class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white text-sm focus:border-amber-500 focus:outline-none @error('answer') border-red-500 @enderror">{{ old('answer', $faq->answer) }}</textarea>
